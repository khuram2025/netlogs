...
```sh
docker build --build-arg SOURCE_COMMIT="$(git rev-parse HEAD)" --build-arg ZENSHIELD_VERSION=0.4.0 -t zenshield:0.4.0 .
...
```sh
python3 scripts/build-ota.py --version 0.4.0 --min-version 0.2.0 --product-id zenai --private-key /secure/release.key --public-key appliance/ota-release.pub --output /releases/ZenShield-0.4.0.zup --source-commit "$(git rev-parse HEAD)" --include-control --changelog "Consolidated upstream application and ZenShield appliance"
python3 scripts/build-native-installer.py --release-package /releases/ZenShield-0.4.0.zup --private-key /secure/release.key --output-dir /releases/installer
```
